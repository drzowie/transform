...
def test_011_QuarticPin():
    x = np.arange(9.).reshape(3,3)
    a = t.QuarticPin()
    d1 = a.apply(x)
    d2 = np.array([[  0.        ,   1.2987013 ,   4.7952048 ],
                   [ 14.68531469,  37.56243756,  82.41758242],
                   [160.63936064, 286.01398601, 474.72527473]])
    assert(  np.allclose(d1,d2, atol=1e-2))

    a = t.QuarticPin(length=5.0)
    d5 = a.apply(x)
    d6 = np.array( [[ 0.        ,  1.02377622,  2.12267732],
                    [ 3.34945055,  4.77602398,  6.49350649],
                    [ 8.61218781, 11.26153846, 14.59020979]])
    assert(  np.allclose(d5,d6, atol=1e-2))

    a = t.QuarticPin(origin=5.0)
    d8 = a.apply(x)
    d9 = np.array( [[52.45254745, 21.78321678,  8.2967033 ],
                    [ 4.2007992 ,  4.1008991 ,  5.        ],
                    [ 6.2987013 ,  9.7952048 , 19.68531469]])

    assert(  np.allclose(d8,d9, atol=1e-2))

    a = t.QuarticPin(strength=6.0)
    d11 = a.apply(x)
    d12 = np.array([[0.00000000e+00, 8.75576037e-02, 7.83410138e-01],
                    [3.24884793e+00, 9.30875576e+00, 2.14516129e+01],
                    [4.28294931e+01, 7.72580645e+01, 1.29216590e+02]])
